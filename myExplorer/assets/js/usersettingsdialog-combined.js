...
function logout() {
    StolenTech.JavaScript.Util.RequestJson(parent.getActionUrl("Logout"), null, function () {
        parent.window.location.href = "login.aspx"
    }, function (n) {
        alert(parent.StolenTech.Util.Language.getEntry("544") + "\n\n" + (n.ImportantMessage || n.Message))
    })
}

function onChangePasswordClick() {
    var n = document.getElementById("CheckBoxChangePassword").checked;
    document.getElementById("LabelPassword").className = n ? "" : "disabled";
    enableElement("Password", n);
    document.getElementById("LabelPasswordConfirm").className = n ? "" : "disabled";
    enableElement("PasswordConfirm", n);
...
StolenTech.JavaScript.Util.OnDomReady(function () {
    ...
    switch (n) {
        case"ChangePasswordOnly":
            elementDialog.onClose = function () {
                logout()
            };
            break;
        case"PasswordChangeComplete":
            ...
            break;
        case"SettingsChangeComplete":
            StolenTech.JavaScript.Util.RefreshPage(parent);
            break;
        case"MessageLogout":
            showClientMessage();
            logout();
